/*
  ...
  V4.B - FULL MANUAL (TASKER OBSOLETE)
...
void processTree_callback(){
  ...
  if(remoteIn == 244){//change condition to status from sensor reading tree (above).
    ...
    //carTime.restart();
    if(irsens1_status == 1 && carTime.elapsed() > 7000){//Possible error when reading carTime.
      //if(carTime.hasPassed(7000)){
      carTime.stop();
      ...
      if(leafA_upState == 1){
        ...
        if(leafA_upState == 1 && remoteIn == 245){
          goto bridgeClose_skip;
        }
      ...
      if(remoteIn == 245){
        ...
        if(leafA_downState == 1){
          ...
          digitalWrite(boatRED, HIGH);//status confirmation, TEMP.
          remoteIn = 0;
        }
      }  
    }      
